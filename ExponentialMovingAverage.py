#！/usr/bin/env python
#_*_coding:utf-8_*_
#以下代码介绍了EzpenentialMovingAverage的使用

import tensorflow as tf

#定义一个变量用于计算滑动平均，这个变量的初始值为0.注意这里手动指定了变量的
#的类型为tf.float32，因为所需要计算滑动平均的变量必须是实数型。
v1= tf.Variable(0,dtype=tf.float32)
#这里step变量模拟神经网络中迭代的轮数，可以用于动态控制衰减率
step = tf.Variable(0, trainable=False)

#定义一个平滑平均的类（class）。初始化时给定了衰减率（0.99）和控制衰减率的变量step。
...
with tf.Session() as sess:
    ...
    #再次更新华东平均值，得到的新华东平均值为0.99*4.555+0.01*10=4.60945.
    sess.run(maintain_averages_op)
    print(sess.run([v1,ema.average(v1)]))#谁出[10.0, 4.6094499]